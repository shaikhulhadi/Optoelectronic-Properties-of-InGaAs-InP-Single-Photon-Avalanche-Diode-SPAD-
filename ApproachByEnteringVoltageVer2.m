...
tou_d=(delta_T-tou)/50 ;  %hold of time of the gate............maximum detrapping time ```````````````(C)....calculated
...
GB=25e9;            %gain bandwidth product
...
I_dm=.1e-6;%%Most reasonable value
V=linspace(25,64,length(Pd_expe));
Pa=linspace(.01,1,length(Pd_expe));
V_BR=65;
n=2;
 ...
 QE(i)=((I_ph(i)./P_)*((h*3e8)/(q_charge*1550e-9)));
   ...
   while (tolarence_pd < delta_pd) && ( tolarence_pon  <  delta_pon )
        if tolarence_pd<delta_pd
            pd_temp=1-exp(-Pa(i)*(I_DM*tou/q_charge + I_DM*(M_o ).^2/(2*pi*q_charge*GB)+...
        ...
        if tolarence_pon < delta_pd
            pon_temp = 1-exp(-Pa(i)*(I_DM*tou/q_charge + I_DM*M_o.^2/(2*pi*q_charge*GB)+...
